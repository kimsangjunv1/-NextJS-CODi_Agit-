...
import BlockEditableComponent from "@/widgets/post/ui/BlockEditable";
            ...
            >
                <section className="flex items-center justify-center bg-white rounded-full shadow-[var(--shadow-normal)]">
                    <UI.Button
                        ...
                        className="text-black rounded-[1.6rem] flex gap-[0.4rem] h-full items-center px-[1.6rem] py-[1.2rem]"
                    >
                        <p className="tablet:block mobile:hidden">아래에 그룹 추가</p>
                        <p>+</p>
                    </UI.Button>
                    ...
                    <UI.Button
                        ...
                        className="text-black rounded-[1.6rem] flex gap-[0.4rem] h-full items-center px-[1.6rem] py-[1.2rem]"
                    >
                        <p className="tablet:block mobile:hidden">그룹 삭제</p>
                        <p>-</p>
                    </UI.Button>
                </section>
            </section>

            <section className="flex flex-1 w-full h-full items-center justify-center gap-[0.8rem] flex-wrap">
                { row.map(( block, blockIndex ) => (
                    <Fragment key={blockIndex}>
                        <section
                            key={ block.id }
                            className="flex-1 min-w-[calc((var(--size-tablet)-(1.6rem*10))/2)] h-full"
                        >
                            
                            <Block block={ block } rowIndex={ rowIndex } blockIndex={ blockIndex } last={ row.length !== 1 } blockCount={ row.length } />
                        </section>
                        
                        <section className={`flex items-center justify-center bg-[var(--color-gray-200)] hover:bg-[var(--color-brand-500)] rounded-[0.8rem] h-full ${ blockIndex === 0 ? "" : "hidden" }`}>
                           <UI.Button
                                onClick={() => {
                                ...
                                className="flex gap-[0.4rem] items-center h-full"
                            >
                                <IconComponent
...
const Block = ({ block, rowIndex, blockIndex, last, blockCount }: { block: SectionContent, rowIndex: number, blockIndex: number, last: boolean, blockCount: number }) => {
    ...
    return (
        ...
        >
            <section className={`relative flex flex-col gap-[1.6rem] min-h-[43.0rem] justify-between h-full ${ block.type === 0 ? "p-[1.2rem]" : "p-[0.4rem]" }`}>
                {/* 액션 */}
                        ...
                        <Fragment>
                            <section className="flex flex-col gap-[0.8rem]">
                                ...
                                <UI.Input
                                    ...
                                    className={{
                                        container: "w-full border-b border-b-[var(--color-gray-200)]",
                                        input: "h-[calc(1.6rem+0.8rem)] tablet:text-[2.4rem] mobile:text-[2.0rem] font-bold text-[var(--color-gray-1000)]"
                                    }}
                                />
                            </section>
                    ...
                    { block.type === 1 ? (
                        <Fragment>
                            <img
                                ...
                                alt="/"
                                className={`object-cover rounded-[2.0rem] w-full ${ blockCount === 1 ? "" : "h-full" }`}
                            />
                            <UI.Button
                ...
                <article className={`flex justify-center action ${ block.type !== 0 ? "mb-[0.8rem]" : "" }`}>
                    <section className="action-inner flex items-center justify-center bg-white p-[0.4rem] rounded-full shadow-[var(--shadow-normal)]">
                        <UI.Button
                            onClick={() =>updateBlock(rowIndex, blockIndex, { type: 0 })}
                            className="text-black rounded-[1.6rem] flex gap-[0.4rem] h-full items-center px-[1.2rem] py-[0.8rem] bg-[var(--color-blue-500)] text-white shadow-[var(--shadow-normal)]"
                        >
                            <p>텍스트</p>
                        </UI.Button>
